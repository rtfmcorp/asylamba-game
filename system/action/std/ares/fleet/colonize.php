...
# send a fleet to colonize a place
...
if ($commanderId !== FALSE AND $placeId !== FALSE) {
	...
	# check si technologie COLONIZE débloquée
	if ($technologies->getTechnology(Technology::COLONIZATION) == 1) {
			...
			if (ASM::$com->size() > 0) {
				...
				if (ASM::$plm->size() > 0) {
					...
					if (CTR::$data->get('playerInfo')->get('actionPoint') >= $PAToTravel) {
						$creditsToTravel = ($obQuantity + $coloQuantity) * CREDITCOEFFTOCOLONIZE;
		...
		CTR::$alert->add('Vous n\'avez pas développé la technologie ' . TechnologyResource::getInfo(Technology::COLONIZE, 'name') . '.', ALERT_STD_ERROR);
	}

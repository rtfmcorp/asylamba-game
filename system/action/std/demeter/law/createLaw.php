...
if ($type !== FALSE && $rColor !== FALSE) {
	if (CTR::$data->get('playerInfo')->get('status') == LawResource::get($type, 'department')) {
		$_CLM = ASM::$clm->getCurrentsession();
		ASM::$clm->load(array('id' => CTR::$data->get('playerInfo')->get('color')));
		if (ASM::$clm->get()->credits >= LawResource::get($type, 'price')) {
			...
			$law = new Law();
			$law->rColorCreator 	= CTR::$data->get('playerInfo')->get('color');
			$law->type 				= $type;
			$law->options 			= $options;
			$law->statement 		= 0;
		} else {
